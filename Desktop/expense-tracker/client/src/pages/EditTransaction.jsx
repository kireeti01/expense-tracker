...
function EditTransaction() {
  const { id } = useParams();
  const { transactions, updateTransaction, categories } = useExpense();
  const navigate = useNavigate();
  const transaction = transactions.find(t => t.id == id);
  const [form, setForm] = useState(transaction || { type: 'expense', amount: '', description: '', category: '', date: '' });
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    updateTransaction(id, { ...form, amount: parseFloat(form.amount) });
